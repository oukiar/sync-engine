...
class Thread(MailSyncBase, HasPublicID, HasRevisions, UpdatedAtMixin,
    ...
    @property
    def most_recent_sender_participant(self):
        ...
        for i in self.messages:
            if sent_recent_date == None:
                recent_participant = i.from_addr
            elif sent_recent_date < i.received_date:
                recent_participant = i.from_addr
    ...
    def most_recent_sent_date(self):
        ...
        for m in sorted_messages:
            if "sent" in [c.name for c in m.categories if c != None] or \
